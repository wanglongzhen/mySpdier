...
class CommLog(object):
    ...
    def debug(self, logmsg):
        ...
        :return:
        """
        strmsg = build_msg(cls_name).format(pnum=self._procnum, msg=logmsg)
...
def build_msg(cls_name):
    if cls_name is None:
        return "[{pnum}] {msg}"
    else:
        return "[{pnum}] [" + cls_name + "] {msg}"


def comm_log(procnum, clss_name=None):
    if clss_name is not None:
        global cls_name
        cls_name = clss_name
